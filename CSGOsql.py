...
def findGameInfo(gameid = None):
    ...
    query = F"SELECT name, adr, team_damage, tk_count, teamflashcount, enemyflashcount, teamflashduration, enemyflashduration FROM gamestats WHERE gameid = {gameid}"
    result = dbconnection.executeQuery(dbconnection.createConnection(), query)
    return result
